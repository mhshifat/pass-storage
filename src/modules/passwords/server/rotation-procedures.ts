...
export const passwordRotationRouter = createTRPCRouter({
  ...
  getReminders: protectedProcedure("password.view")
    ...
    .query(async ({ input, ctx }) => {
      ...
      for (const password of passwords) {
        ...
        // Check if reminder is within the specified days ahead
        if (reminderDate <= futureDate && reminderDate >= now) {
          reminders.push({
            ...
            nextRotationDate,
            reminderDate,
            daysUntilRotation: Math.ceil((nextRotationDate.getTime() - now.getTime()) / (24 * 60 * 60 * 1000)),
            daysUntilReminder: Math.ceil((reminderDate.getTime() - now.getTime()) / (24 * 60 * 60 * 1000)),
          })
        }
      }
  ...
  completeRotation: protectedProcedure("password.edit")
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const encryptedPassword = encrypt(input.newPassword)
      ...
      await savePasswordHistory({
        passwordId: rotation.passwordId,
        password: encryptedPassword,
